    ...
    try:
        for batch_idx, batch in enumerate(pbar):
            ...
            with autocast():
                ...
                l1_loss = criterion(output, target_img)   #核心损失计算的地方
                # SSIM损失计算（需转换数值范围到[0,1]）
                output_scaled = output * 0.5 + 0.5  # [-1,1] => [0,1]
                target_scaled = target_img * 0.5 + 0.5
                # 严格按图中公式计算：Σ(1 - SSIM_pixel)/N
                # 使用标准ssim函数+nonnegative参数
                ssim_map = ssim(output_scaled, target_scaled,
                                data_range=1.0,
                                size_average=False,
                                win_size=7,  # 可调参数
                                )  # 高斯核标准差
                # 手动添加非负约束
                ssim_map = torch.clamp(ssim_map, min=0.0)  # 强制SSIM≥0
                ssim_loss = (1.0 - ssim_map).mean()  # <- 等价于 Σ(1 - SSIM)/N
                # 复合损失（建议权重λ=0.5~1.5）
                loss = l1_loss + 0.01 * ssim_loss  # <- 修改此处
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(save_dir, args.model + '.pth')):
        print('==> Start training, model,current model name: ' + args.model)
